...
# Configuration
$pluginId = "wrservices-statusoverview-panel"
...
$packageDir = Join-Path -Path $rootDir -ChildPath $OutputFolder
$zipPath = "$packageDir\$pluginId-$Version.zip"

# Check if the package exists
if (-not (Test-Path $zipPath)) {
    Write-Host "❌ Package not found: $zipPath" -ForegroundColor Red
    Write-Host "Please run 'plugin.ps1 package' first or specify the correct version." -ForegroundColor Red
    exit 1
}
...
try {
    ...
    $fixedZipPath = "$packageDir\$pluginId-$Version-fixed.zip"
    ...
    # Run the analyze script to verify fixed package
    $analyzeScript = Join-Path -Path $PSScriptRoot -ChildPath "analyze.ps1"
    Write-Host "`nVerifying fixed package structure:" -ForegroundColor Cyan
    & $analyzeScript -ZipPath $fixedZipPath -Brief
